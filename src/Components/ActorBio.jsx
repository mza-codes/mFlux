...
const ErrMsg = ({ route }) => (
  <>
    ...
    >
      <iconify-icon icon="ic:round-arrow-circle-left" width="33" height="33" />{' '}
      &nbsp;Go Back
    </h2>
  </>
);
...
const ActorBio = ({ actor }) => {
  ...
  if (!actor?.id || !actor?.name) {
    return <ErrMsg route={goBack} />;
  }
  if (loading) {
    return (
      <h1 className=" text-6xl py-8 font-righteous text-center text-teal-400">
        Loading..
      </h1>
    );
  }
  return (
    <section className=" text-white py-6 actorWrapper flex flex-row flex-wrap justify-evenly ">
      <div className="actorImage sm:w-full md:w-1/2 lg:w-1/2 max-w-md min-w-[280px] p-3 max-h-fit relative">
        <div className="flex items-center justify-center">
          <div className="relative">
            ...
            <div
              className="icon text-rose-500 hover:text-rose-600 z-[104] opacity-0 hover:opacity-100
      ...
      <div
        className="profile min-w-[280px] xl:max-w-[46vw] text-center ml-2 lg:max-w-[46vw] px-3
      ...
      >
        ...
        <div className="links flex flex-row gap-3 justify-center xl:justify-start lg:justify-start sm:justify-center">
          ...
          >
            <iconify-icon icon="fa:imdb" width="33" height="33" />{' '}
          </a>
          {actor?.homepage && (
            <a
              href={`${actor?.homepage}`}

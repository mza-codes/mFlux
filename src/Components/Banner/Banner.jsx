    ...
    <>
      <Slider {...settings}>
        {items.map((item, i) => (
          <main className="Banner" key={item?.id ?? i}>
            <LazyLoad offset={100}>
              <div
                className="bg xl:h-[100vh] lg:h-[80vh] md:h-[70vh] sm:h-[66vh] h-[68vh]"
                style={{
                  backgroundImage: `url(${
                    POSTER_URL +
                    (item?.backdrop_path
              ...
              >
                <div className="content capitalize sm:w-full lg:w-1/2 w-full">
                  ...
                  <h5 className="font-light font-abel text-xl p-3">
                    {item?.overview || ''}
                  </h5>
                ...
                <div className="changeBtn">
                  <div
                    title="Favourite This"
                    className="icon text-rose-500 hover:text-rose-600
        ...
        <>
          <Suspense
            fallback={
              <>
                {' '}
                <h2 className="text-center font-righteous text-6xl text-yellow-100 p-4 m-4">
                  Loading...
                </h2>{' '}
              </>
            }
          >
            <HorizRow
              title={`Displaying ${results?.length} titles for "${query}" `}
              data={results}
              close
            />
          </Suspense>
      ...
      {!gotResult && !isClosed && err && (
        <div className="w-full">
          {' '}
          <h2 className="font-righteous text-2xl p-4 text-red-400">
            {err}
          </h2>{' '}
        </div>
      )}
